SELECT DISTINCT
	...
	,ISNULL(CAST((((SELECT SUM(CH) FROM SHISTDISCCONCLUIDAS (NOLOCK) WHERE RA = SMATRICPL.RA AND IDHABILITACAOFILIAL = SHABILITACAOALUNO.IDHABILITACAOFILIAL) + (SELECT ISNULL(SUM(CH),0) FROM SHISTDISCOPTELETIVAS (NOLOCK) WHERE RA = SMATRICPL.RA AND IDHABILITACAOFILIAL = SHABILITACAOALUNO.IDHABILITACAOFILIAL AND STATUS = 'Aprovado')) * 100 / SGRADE.CARGAHORARIA) AS DECIMAL(5,1)),0) AS NU_PERCENTUAL_INTEGRALIZACAO
	, '2023' AS 'NU_ANO_FORMATURA'
    , '2' AS 'NU_SEMESTRE_FORMATURA'
    ,'2023' AS NU_ANO_INICIO_GRADUACAO
    , '0' AS 'IN_MUNICIPIO_POLO_EXTERIOR'
    , 'CO_MUNICIPIO_POLO' AS 'CO_MUNICIPIO_POLO'
...
FROM
	...
	LEFT JOIN SHISTHABILITACAOALUNO (NOLOCK)
	...
	LEFT JOIN SHISTDISCCONCLUIDAS (NOLOCK)
...
WHERE
	SCURSO.CODCURSO = '03'	
	and SMATRICPL.RA like '23%'
	AND SMATRICPL.CODSTATUS IN (14,43)
	AND SHABILITACAOFILIAL.CODFILIAL = 01
	AND SHABILITACAOALUNO.CODSTATUS != 10
	--AND SMATRICPL.IDPERLET = 87
	--AND SMATRICPL.RA = 'E22-2-18473'
	--AND CAST((((SELECT SUM(CH) FROM SHISTDISCCONCLUIDAS (NOLOCK) WHERE RA = SMATRICPL.RA AND IDHABILITACAOFILIAL = SHABILITACAOALUNO.IDHABILITACAOFILIAL) + (SELECT ISNULL(SUM(CH),0) FROM SHISTDISCOPTELETIVAS (NOLOCK) WHERE RA = SMATRICPL.RA AND IDHABILITACAOFILIAL = SHABILITACAOALUNO.IDHABILITACAOFILIAL AND STATUS = 'Aprovado')) * 100 / SGRADE.CARGAHORARIA) AS DECIMAL(6,2)) >= 80.00
